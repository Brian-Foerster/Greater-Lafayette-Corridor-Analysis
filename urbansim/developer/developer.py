...
class Developer(object):
    ...
    def pick(self, form, target_units, parcel_size, ave_unit_size,
             ...
             profit_to_prob_func=None, deterministic=False):
        """
        ...
        deterministic : bool (default False)
            If True, select parcels in descending order of profit per size
            instead of using a stochastic draw.  Produces reproducible results
            and is more realistic for small parcel sets (<100) where the
            stochastic draw introduces high variance.
        ...
        if df.net_units.sum() < target_units:
            ...
            # Deterministic: select most profitable parcels first.
            # More realistic for small parcel sets and eliminates
            # stochastic variance across runs.
            df_sorted = df.sort_values('max_profit_per_size', ascending=False)
            tot_units = df_sorted['net_units'].values.cumsum()
            ind = int(np.searchsorted(tot_units, target_units, side="left")) + 1
            build_idx = df_sorted.index.values[:ind]
        else:
            # Stochastic: weighted random draw (original behavior).
            # We don't know how many developments we will need, as they differ in net_units.
            # If all developments have net_units of 1 then we need target_units of them.
